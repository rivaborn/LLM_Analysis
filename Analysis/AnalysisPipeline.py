...
class PipelineStep:
    ...
    script: str            # Filename, e.g. "archgen.ps1"
...
PIPELINE_STEPS = [
    PipelineStep("Per-file docs",        "archgen.ps1",           ["-Preset", "unreal"], use_target_dir=True,  is_powershell=True),
    ...
    PipelineStep("Architecture overview", "arch_overview.ps1",     [],                    use_target_dir=False, is_powershell=True),
    PipelineStep("Pass 2 context",        "archpass2_context.ps1", [],                    use_target_dir=False, is_powershell=True),
    PipelineStep("Pass 2 analysis",       "archpass2.ps1",         [],                    use_target_dir=False, is_powershell=True),
]
...
def rename_architecture_folder(repo_root: Path, index: int, subsection: str, logger: logging.Logger, dry_run: bool = False) -> None:
    src = repo_root / "architecture"
    sanitized = sanitize_subsection_name(subsection)
    dst = repo_root / f"{index}. {sanitized}"
    ...
    if not src.exists():
        raise FileNotFoundError(f"architecture/ not found at {src}")
...
def run_pipeline(script_dir: Path, repo_root: Path, subsections: list[str], logger: logging.Logger, dry_run: bool = False, start_from: int = 1) -> None:
    ...
    for i, subsection in enumerate(subsections, start=1):
        ...
        rename_architecture_folder(repo_root, i, subsection, logger, dry_run)
