...
```
python LLM_Analysis\Analysis\AnalysisPipeline.py [OPTIONS]
```

**Always launch from the target repo root** (typically a UE checkout) — the orchestrator uses `Path.cwd()` as the repo root and passes that as `cwd=` to every subprocess. The PS1 worker scripts use the same convention (`(Get-Location).Path`), so the subsection paths from `.env` (e.g. `Engine/Source/Runtime/Core`, `Engine/Source/Runtime/Renderer`) are resolved relative to that CWD. Use `--repo-root <path>` to override.
...
| # | Step                  | Script                                                       | Notes                                                                                                                                                |
| - | --------------------- | ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | Per-file docs         | `archgen.ps1 -TargetDir <subsection> -Preset unreal` | LLM call. The `-Preset unreal` arg is hardcoded in the PIPELINE_STEPS list — change it there if you want a different preset for orchestrated runs. |
| 2 | Cross-reference index | `archxref.ps1`                                               | Pure text, no LLM.                                                                                                                                   |
| 3 | Mermaid diagrams      | `archgraph.ps1`                                              | Pure text, no LLM.                                                                                                                                   |
| 4 | Architecture overview | `arch_overview.ps1`                                    | LLM call.                                                                                                                                            |
| 5 | Pass 2 context        | `archpass2_context.ps1`                                      | Pure text, no LLM.                                                                                                                                   |
| 6 | Pass 2 analysis       | `archpass2.ps1`                                        | LLM call.                                                                                                                                            |
...
If you've already generated these (or you're running on a non-C++ subsection), pass `--skip-lsp` to bypass them.
...
This means if a previous run completed `1. Engine/Source/Runtime/Core` and `2. Engine/Source/Runtime/CoreUObject` but failed on `Engine/Source/Runtime/Engine`, you can simply re-run with no flags and it will resume at subsection 3.
...
- **Subsections** — the lines between `#Subsections begin` and `#Subsections end`. Comment lines (starting with `#`) and blanks are ignored. Forward and back slashes are both preserved (`Engine/Source/Runtime/Core` and `Engine\Source\Runtime\Core` both valid).
- **Claude CLI model, account dirs, retries, file filters** — read by the worker PS1 scripts via `Common/llm_common.ps1`. AnalysisPipeline.py itself does not read these; it only reads the subsections block.
- **Preset** — also read by the worker PS1s; `PIPELINE_STEPS[0]` hardcodes `-Preset unreal` for orchestrated runs.

Example subsection block (UE 5.x runtime subsystems, pre-populated in `.env`):
...
# Reflection, GC, UObject system
Engine/Source/Runtime/Engine
# Actors, components, world, gameplay framework
Engine/Source/Runtime/Renderer
# Deferred shading, scene rendering
... (10 more subsystems)
#Subsections end
```

Resulting renamed output folders after a clean run: `1. Engine_Source_Runtime_Core/`, `2. Engine_Source_Runtime_CoreUObject/`, `3. Engine_Source_Runtime_Engine/`, etc. (slashes are sanitized to underscores).
...
```
python C:\Coding\LLM_Analysis\Analysis\AnalysisPipeline.py --dry-run
```

**Skip LSP setup (you already have `compile_commands.json` and `.serena_context.txt` files):**
```
python C:\Coding\LLM_Analysis\Analysis\AnalysisPipeline.py --skip-lsp
```

**Resume after a failed run on subsection 3 (skip-detect handles 1 and 2, but you want to be explicit):**
```
python C:\Coding\LLM_Analysis\Analysis\AnalysisPipeline.py --start-from 3 --skip-lsp
```

**Re-run only the C# editor subsection (it's third in the list):**
```
# Delete the existing "3. Engine_Source_Runtime_Engine/" first, then:
python C:\Coding\LLM_Analysis\Analysis\AnalysisPipeline.py --start-from 3 --skip-lsp
```
...
- The 6 worker scripts: `archgen.ps1`, `archxref.ps1`, `archgraph.ps1`, `arch_overview.ps1`, `archpass2_context.ps1`, `archpass2.ps1`
