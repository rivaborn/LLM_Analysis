...
`archxref.ps1` parses the per-file architecture documents produced by `archgen.ps1` and builds
...
This cross-reference index is consumed by downstream pipeline steps -- `arch_overview.ps1`,
`archpass2_context.ps1`, and `archpass2.ps1` -- to provide relationship context that a
...
| Per-file docs              | Output of `archgen.ps1` in `<repo>/architecture/` |
...
.\Analysis\archxref.ps1
```

**Via AnalysisPipeline.py:**
Called as the second analysis step (after `archgen.ps1`):
```
...
| ---- | ------------------------------------------------------ |
| `0`  | Success                                                |
| `1`  | No per-file docs found (run `archgen.ps1` first) |
