...
(`archpass2.ps1`). Rather than feeding the entire `architecture.md` and `xref_index.md` to
...
| `architecture/architecture.md` | Output of `arch_overview.ps1` (or any `*architecture.md` files) |
| `architecture/xref_index.md`   | Output of `archxref.ps1` (or any `*xref_index.md` files)              |
| Per-file docs                  | Output of `archgen.ps1` in `architecture/`                      |
...
.\Analysis\archpass2_context.ps1
```

**Via AnalysisPipeline.py:**
Called as the fifth analysis step (after `archgen.ps1`, `archxref.ps1`, `archgraph.ps1`,
`arch_overview.ps1`):
```
