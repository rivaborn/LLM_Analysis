...
- Three Claude config directories:
  ...
  - `$env:USERPROFILE\.claudemain`
  - `$env:USERPROFILE\.claudealt` (active session, via `CLAUDE_CONFIG_DIR`)
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "sh /c/Users/rivaborn/.claude<suffix>/statusline-command.sh"
  }
}
```
...
Files fixed:
- `C:\Users\rivaborn\.claude\statusline-command.sh`
- `C:\Users\rivaborn\.claudemain\statusline-command.sh`
- `C:\Users\rivaborn\.claudealt\statusline-command.sh`

## Verification
```bash
# Test with sample JSON - no stderr, correct output:
echo '{"model":{"display_name":"Opus 4.6 (1M context)","id":"claude-opus-4-6"},...}' \
  | sh /c/Users/rivaborn/.claudealt/statusline-command.sh
...
### Root Cause
A **project-level** `.claude/settings.json` in the repo root had its own `statusLine` config pointing to a minimal script at `.claude/statusline-command.sh`. This script only output model name + context window (no cost or rate limits). Project-level settings override user-level settings, so the full script in `~/.claudealt/statusline-command.sh` was never executed.
...
- `~/.claudemain/statusline-command.sh`
- `~/.claudealt/statusline-command.sh`
